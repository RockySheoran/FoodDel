  ...
  return (
    <>
      <div className="container mx-auto p-4" id="menu">
        <h1
          className={`font-bold ${
            isDarkTheme ? "text-white" : "text-[#262626]"
          }`}>
          Explore our menu
        </h1>
        <p
          className={`my-3 lg:w-[700px] px-1 md:px-3 lg:px-4 ${
            isDarkTheme ? "text-gray-300" : "text-[#262626]"
          }`}>
          ...
          {[...menu_list, ...menu_list].map((dish, index) => {
            return (
              <div
                onClick={() =>
                  setcategory((pre) =>
                    pre === dish.menu_name ? "All" : dish.menu_name
                  )
                }
                key={index}
                className={`flex flex-col items-center  flex-shrink-0 overflow-visible rounded-lg  ${
      ...
      <hr
        className={`h-1 my-3 container ${
          isDarkTheme ? "bg-gray-600" : "bg-[#e2e2e2]"
        }`}
      />
    </>
  );
