...
const MyOrder = () => {
  ...
  return (
    <div
      className={`my-order  md:mx-12 container ${
        ...
        <div className="container flex flex-col gap-5 mt-8 my-3 justify-center">
          {data.map((order, index) => {
            return (
              <div
                key={index}
                className={`my-orders grid lg:grid-cols-8 grid-cols-4 gap-2 lg:gap-8 border-2 px-1 md:px-2 items-center ${
                  isDarkTheme
                ...
                <p className="col-span-2">
                  ...
                  <b
                    className={`font-bold ${
                      isDarkTheme ? "text-gray-300" : "text-[#454545]"
                    }`}>
                    {order.status}
                  </b>
                </p>
                <button
                  onClick={fetchOrders}
                  className={`relative bottom-1 text-[10px] md:text-xl border-2 rounded-md h-8 ${
                    isDarkTheme
