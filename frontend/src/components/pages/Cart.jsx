...
const Cart = ({ loginCheck, setLoginCheck }) => {
  const { cartItem, removeCart_Item, food_list, totalCartItemPrice, url, token } = useContext(StoreContext);
      ...
      <div className="Cart_Total grid grid-cols-1 md:grid-cols-2 px-3">
        <div className="Promo_code my-4">
          ...
          <div className="code">
            <input
              type="text"
              className={`rounded-s-md h-8 pl-2 ${isDarkTheme ? 'bg-gray-700 text-white placeholder-gray-400' : 'bg-slate-400 text-black placeholder-slate-300'}`}
              placeholder="Promo code"
            />
            <button
              type="button"
              className={`px-2.5 rounded-e-md h-8 ${isDarkTheme ? 'bg-gray-800 text-white' : 'bg-black text-white'}`}
            >
              Submit
            </button>
          </div>
