...
const FeedbackDisplay = () => {
  ...
  return (
    <div className={`w-full mb-7 bg-gray-100  rounded-lg py-4  ${isDarkTheme ? 'bg-gray-700 ' : ' bg-slate-50 '}`}>
      <h2 className="text-center text-xl font-bold mb-4">User Feedback</h2>
      <div className="overflow-x-scroll relative">
        <div  ref={scrollRef} className=" flex !overflow-x-auto  gap-6 p-4 "   style={{ scrollbarWidth: "none", whiteSpace: "nowrap" }}>
          {[...feedbacks,...feedbacks].map((feedback, index) => {
          return  (<div
              key={index}
              className={`flex flex-col w-[300px] px-2 py-1 hover:scale-105 hover:translate-y-1.5    flex-shrink-0 overflow-visible rounded-lg  ${isDarkTheme ? 'bg-slate-600' : 'bg-slate-100'}`}

            >
              <h3 className="text-lg font-semibold">{feedback.name}</h3>
              <p className={`text-sm text-wrap ${isDarkTheme ? 'text-white' : 'text-black'} `}>{feedback.comments}</p>
