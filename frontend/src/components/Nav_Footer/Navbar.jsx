...
const Navbar = ({ setLoginCheck }) => {
  ...
  return (
    ...
    <nav className={` fixed overflow-y-hidden w-full top-0 z-[99999999] h-10 pt-1 ${isDarkTheme ? 'bg-dark text-white' : 'bg-slate-50 text-black shadow-lg' }`}>
...
<nav className={`bg-gray-900 text-white overflow-y-hidden fixed top-0 left-0 w-full z-50 shadow-md ${isDarkTheme ? 'bg-dark text-white' : 'bg-slate-50 text-black shadow-lg' }`}>
      <div className="max-w-7xl mx-auto px-4  overflow-y-hidden flex justify-between items-center h-14">
        ...
        <figure className="figure relative top-2 ">
            ...
            <Link to="/">
              <img src={assets.logo} className="figure-img  rounded !h-8 w-24 md:w-full pl-2 md:pl-1" alt="logo" />
            </Link>
          </figure>
        </div>
    ...
    isDarkTheme ? "animate-spin-slow hover:scale-105" : ""
        ...
        >
          ...
          <FiAlignJustify color={`${isDarkTheme ? 'white' : 'black'}`} className="w-6 h-6 text-current" />
        </button>
        </div>
      </div>
    ...
    >
    <PiHandHeartFill className="w-6 h-6 mr-2" /> <span> Favorites</span>
    </Link>
    <Link to="/cart" className={`block flex gap-2 px-4 py-2 rounded no-underline ${
        isDarkTheme ? 'bg-dark text-white' : ' bg-white text-black'
      }`}>
            ...
            >
             ...
             <span> Sign In</span>
               ...
               <Link to="/myorders"
                   ...
                   className={`order1 flex mb-0.5 items-center gap-2 py-2 px-3 hover:text-red-600 rounded content-center cursor-pointer no-underline ${isDarkTheme ? 'bg-dark text-white' : ' bg-white text-black'}`}
                  >
                    <img src={assets.bag_icon} alt="order" className="h-5" />
                    <span className={`${isDarkTheme ? ' text-white hover:text-red-600' : ' text-black hover:text-red-600'} hover:text-red-600`}>Order</span>
                  </Link>

             
                  <p
                    className={`flex items-center rounded gap-2 px-3 py-2 cursor-pointer ${isDarkTheme ? 'bg-dark text-white' : ' bg-white text-black'}`}
                    onClick={isLogOut}
                  >
                    <img src={assets.logout_icon} alt="logout" className="h-5" />
                    <span className={`${isDarkTheme ? ' text-white hover:text-red-600' : ' text-black hover:text-red-600'} !hover:text-red-600`}>Logout</span>
                  </p>
                  </div>)
