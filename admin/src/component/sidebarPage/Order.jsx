      ...
      <div className="order_list flex flex-col gap-3">
        {orders.map((order, index) => {
          return (
            <div
              key={index}
              className={`order-item grid pt-4 gap-2 xl:gap-4 px-2 border-2 ${isDarkTheme ? 'border-gray-700 text-white' : 'border-slate-500'} text-[#505050] pr-2`}
            >
              ...
              <select
                ...
                value={order.status}
                className={`border-2 rounded-md ${isDarkTheme ? 'bg-gray-800 border-gray-600 text-white' : 'bg-[#ffe8e4] border-red-200 text-black'}`}
              >
